...
public class ArrayAct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(text==null||text.equals("마감일자순")) {
			act = new EventService().selectAllEventAct(cPage, numPerpage);
		}else if(text.equals("인기도순")) {
			act = new EventService().selectAllEventActLike(cPage, numPerpage);
		}else if(text.equals("리뷰많은 순")) {
			act = new EventService().selectAllEventActReview(cPage, numPerpage);
