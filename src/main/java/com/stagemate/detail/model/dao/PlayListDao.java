...
public class PlayListDao {
	...
	public List<Detail> selectPlayDetailCondition(Connection conn, String userId, String day, String yyyy, String mm, String status, int startRow, int endRow){
		...
		List<Detail> result = new ArrayList();
		try { 
			String query = "SELECT * FROM (SELECT EVENT_ORDER_TB.*, EVENT_TB.EVENT_NM, ROW_NUMBER() OVER (ORDER BY EVENT_ORDER_TB.RSV_DATE DESC) AS RN " + 
					 "FROM EVENT_ORDER_TB JOIN EVENT_TB ON EVENT_ORDER_TB.ES_NO = EVENT_TB.EVENT_NO " + 
					 "WHERE (RSV_DATE >= CASE " +
